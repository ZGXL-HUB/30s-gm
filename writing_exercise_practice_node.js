...
class WritingExercisePracticeNode {
  constructor() {
    this.questions = writingExerciseQuestions;
    this.progressFile = path.join(__dirname, 'writing_exercise_progress.json');
  ...
  loadProgress() {
    try {
      if (fs.existsSync(this.progressFile)) {
        const savedProgress = fs.readFileSync(this.progressFile, 'utf8');
  ...
  saveProgress() {
    try {
      fs.writeFileSync(this.progressFile, JSON.stringify(this.userProgress, null, 2));
    } catch (error) {
