...
二、常见名词复数变化规则及示例的表格：
|规则分类|变化规则|示例(单数→复数)|
|一般情况|直接加 -s|cat→cats, book→books|
|以s/x/sh/ch结尾|加 -es|box→boxes, watch→watches|
|以辅音字母+y结尾|变y为i再加-es|baby→babies, city→cities|
|以f/fe结尾|变f/fe为v再加-es|leaf→leaves, knife→knives|
|不规则变化|需单独记忆|child→children, foot→feet|
...
五、修饰词提示
题目：She has many ______ (book) in her room.
答案：books(修饰词 many 提示用复数，符合 "一般情况加 -s" 规则)`;
...
function mergeItemsInArray(items) {
  ...
  while (i < items.length) {
    ...
    if (isCycleStart(currentItem)) {
      const cycleGroup = extractCycleGroup(items, i);
      if (cycleGroup) {
