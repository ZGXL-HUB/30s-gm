...
Page({
  data: {
    activityId: '',
    sectionTitle: '第二环节：标志词识别，请点击名词复数的标志词',
    groups: [],
    groupIndex: 0,
    sentenceIndex: 0,
    sentences: [],
  ...
  onLoad(options) {
    ...
    const groups = liveService.getSignalWordGroups(activityId);
    ...
    wx.setNavigationBarTitle({ title: '第二环节·标志词识别' });
  },
  ...
  nextGroup() {
    ...
    if (next >= groups.length) {
      ...
      liveService.saveResult(this.data.activityId, liveService.SEGMENT_TYPES.SIGNAL_WORD_RECOGNITION, { completed: true });
      sound.playToast();
      this.setData({
        allDone: true,
  ...
  skipSegment() {
    ...
    this.setData({
      ...
      skipConfirmMsg: isLastGroup ? '恭喜你完成所有标志词识别题！' : '是否进入下一组标志词识别？'
    });
  },
  ...
  goToNextSegment() {
    const nextType = liveService.getNextSegmentType(liveService.SEGMENT_TYPES.SIGNAL_WORD_RECOGNITION, this.data.activityId);
